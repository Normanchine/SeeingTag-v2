...
def load_config(path: str) -> dict:
    ...
    return {
        ...
        "filter_alpha_yaw": cfg.get("filter_alpha_yaw", cfg.get("filter_alpha", 1.0)),
        "output_filter_alpha": cfg.get("output_filter_alpha", 1.0),
        "output_filter_alpha_yaw": cfg.get("output_filter_alpha_yaw", cfg.get("output_filter_alpha", 1.0)),
        ...
        "blind_keep_armed_default": cfg.get("blind_keep_armed_default", False),
        "camera_index": cfg.get("camera_index", None),
...
class UdpSender:
    def __init__(self, target_ip: str, target_port: int, output_filter_alpha: float = 1.0,
                 debug_logging: bool = False, output_filter_alpha_yaw: float = 1.0):
        self.target = (target_ip, target_port)
        self.sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.seq = 0
        self.output_filter_alpha = float(np.clip(output_filter_alpha, 0.0, 1.0))
        self.output_filter_alpha_yaw = float(np.clip(output_filter_alpha_yaw, 0.0, 1.0))
    ...
    def send(self, x: float, z: float, yaw: float = 0.0, tracking_state: str = "raw") -> bool:
        ...
        try:
            ...
            if self.filtered_output is None:
                ...
                alpha_yaw = self.output_filter_alpha_yaw
                self.filtered_output = (
                    old_x + alpha * (x - old_x),
                    old_z + alpha * (z - old_z),
                    smooth_angle(old_yaw, yaw, alpha_yaw),
                )
...
class TagTracker:
    ...
    def _marker_size_px(corners: np.ndarray) -> float:
        """用四条图像边的像素长度估算 Tag 尺寸，不受平面旋转角度影响。"""
...
def run_calibration(cfg: dict):
    ...
    cap = open_camera(cfg.get("camera_index", None))
...
def run_competition(cfg: dict):
    ...
    blind_keep_armed = cfg.get("blind_keep_armed_default", False)
    if blind_keep_armed:
    ...
    visual_filter_alpha_yaw = float(np.clip(cfg["filter_alpha_yaw"], 0.0, 1.0))
        ...
        if position_detected:
            ...
            if visual_filtered_position is None:
                ...
                visual_filtered_position = (
                    ...
                    smooth_angle(old_yaw, car_yaw if car_yaw is not None else old_yaw, visual_filter_alpha_yaw),
                )
