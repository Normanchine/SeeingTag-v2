...
当前版本已调整为使用四个固定 Tag 的**中心点**建立 Homography，因此固定 Tag 可按现场识别效果旋转摆放，无需统一码面方向。系统同时支持原图优先识别车载 Tag、鸟瞰图兜底识别、短暂丢失位置保持、H 矩阵缓存保护和分层平滑输出。

完整的流程、配置项、UDP 状态字段与现场调参建议，请参阅 [本次定位优化说明](本次定位优化说明.md)。
